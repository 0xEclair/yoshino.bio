...
1. for align
  ...
  *-end note*]
  ...
  1.*影响：* 如果可以通过对齐带有长度space的ptr指向的buffer来对齐存储的size字节,
  该函数将ptr更新为指向此类存储的第一个可能的地址,并通过对齐所使用的字节数减少space.否则，该函数不做任何事.

  2.*要求：* 
   - 对齐方式应为基本对齐方式值或此实现中支持的扩展对齐方式值.
  ...
  12.*影响：* ::new((void*)p) U(std::forward<Args\>(args)...)
  ...
  4.*影响：* 在迭代器指向的地方从element构造一个值.

  5.*返回值：* iterator的引用.

  `raw_storage_iterator<OutputIterator,T>& operator++();`

  6.*影响：* 前++:advance迭代器并返回对更新后的迭代器的引用.

  `raw_storage_iterator<OutputIterator,T> operator++(int);`

  7.*影响：* 后++:advance迭代器并返回对更新前的迭代器的引用.

  ### 20.6.11 Temporary buffers

  `template <class T> pair<T*, ptrdiff_t> get_temporary_buffer(ptrdiff_t n) noexcept;`

  1.*影响：* 获得足以保存n个相邻T对象的存储空间的指针.由实现定义是否支持过度对齐(over-aligned)的类型.
  ...
  [*Note:* 这使得现有的C库不受安全派生的指针的限制影响,但代价是为malloc()分配的对象提供少得多的垃圾回收和泄漏检测选项.它还允许malloc()通过单独的分配空间来实现,从而绕过常规的clarify_reachable()实现.
...
1. for align
  ...
  1.*影响：* 从另一个default_delete<U\>对象构造一个default_delete对象.
  ...
  2.*影响：* 构造一个unique_ptr对象, 其不用有任何内容, 值初始化存储的指针和deleter.
  ...
  6.*影响：* 构造一个unique_ptr对象, 其拥有p, 用p初始化存储的指针, 值初始化存储的deleter.
  ...
  13.*影响：* 构造一个unique_ptr对象, 其持有p, 用p初始化存储的指针, 按如上所述初始化deleter.
  ...
  16.*影响：* 通过从u转移所有权到*this来构造unique_ptr. 如果D是引用类型, 这个deleter从u的deleter copy构造;否则, 移动构造.
  ...
  20.*影响：* 通过从u转移所有权到*this来构造unique_ptr. 如果E是引用类型, 这个deleter从u的deleter copy构造;否则, 移动构造.
  [*Note:* deleter构造函数可能用std::forward<D\>实现. *-end note*]
  ...
  2.*影响：* 如果 get() == nullptr, 无影响. 否则, get_deleter()(get()).
  ...
  2.*影响：* 将所有权从u转移到*this, 如同以调用reset(r.release())后调用get_deleter()到std::forward<E\>(r.get_deleter())的赋值.
  ...
  6.*影响：* 将所有权从u转移到*this, 如同以调用reset(r.release())后调用get_deleter()到std::forward<E\>(r.get_deleter())的赋值.
  ...
  4.*影响：* 用p对存储的指针赋值, old_p表示旧值且不等于nullptr, 调用get_deleter()(old_p).
  ...
  1.*影响：* 如果get() == nullptr, 无影响;否则, get_deleter()(get()).
  ...
  use_count()中的更改不会影响可能引起数据竞争的修改.

  ##### 20.7.2.2.1 shared_ptr constructors

  `constexpr shared_ptr() noexcept;`

  1.*影响：* 构造一个empty shared_ptr对象.
  ...
  9.*影响：* 构造一个持有指针p和deleter d的shared_ptr对象.
  ...
  13.*影响：* 构造存储p的shared_ptr对象, 和r共享所有权.
  ...
  18.*影响：* 如果r empty,构造一个empty shared_ptr对象; 否则, 构造一个和r共享所有权的shared_ptr对象.
  ...
  24.*影响：* 构造与r共享所有权的shared_ptr, 其存储r中存储的指针的copy.

  25.*后置条件：* use_count() == r.use_count()

  26.*Throws:* 当r.expired(), bad_weak_ptr

  27.*异常安全：* 如果抛出异常, 此构造函数无影响.
  ...
  33.*影响：* 当D不是引用类型时,等价于shared_ptr(r.release(), r.get_deleter());
  否则shared_ptr(r.release(), ref(r.get_deleter()))

  34.*异常安全：* 如果抛出函数, 此构造函数无影响.
